...
st.caption(
"Ingresa precio en UF, cantidad y un **monto de descuento en CLP**. La app calcula el % equivalente, valida topes por nivel y muestra el total en CLP."
)
...
with col_flash:
activar_flash = st.toggle("Ofertas Flash", value=False, help="Activa para ampliar topes por nivel en días especiales")
...
if excede_tope:
st.error(
f"El monto solicitado {formato_clp(monto_descuento_ing)} excede el tope permitido para {nivel} (máx {int(max_desc*100)}% = {formato_clp(monto_tope)}). Se aplicará el tope.",
icon="⛔",
)
...
st.caption("Fuente UF: mindicador.cl · La app ahora usa **monto en CLP** en vez de % y valida topes por nivel (Nivel 1 = 25%, Telecierre = 40%). Con **Ofertas Flash** puedes ajustar temporalmente esos topes.")
